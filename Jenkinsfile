pipeline {
    agent {
        kubernetes {
            label 'docker-agent' // Etichetta per identificare il pod
            defaultContainer 'docker' // Nome del container predefinito in cui eseguire i comandi
            containers {
                containerTemplate(
                    name: 'docker',
                    image: 'docker:20.10.24-dind', // Usa un'immagine che supporta Docker-in-Docker
                    ttyEnabled: true,
                    command: 'cat',
                    privileged: true  // Permette di eseguire Docker in Docker
                )
            }
        }
    }
